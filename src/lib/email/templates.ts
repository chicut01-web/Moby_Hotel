...
function shell(title: string, bodyHtml: string): string {
  return `<!doctype html>
<html>
<body style="margin:0;padding:0;background:#F1F3F4;font-family:Inter,'Helvetica Neue',Arial,sans-serif;color:#214E6D;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F1F3F4;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #DDEAF0;border-radius:12px;overflow:hidden;">
        <tr>
          <td style="padding:28px 32px;border-bottom:1px solid #DDEAF0;">
            <p style="margin:0;font-family:Inter,'Helvetica Neue',Arial,sans-serif;font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#357592;">Hub for</p>
        ...
        <tr>
          <td style="padding:20px 32px;border-top:1px solid #DDEAF0;font-family:Inter,'Helvetica Neue',Arial,sans-serif;font-size:11px;color:#4A5F6E;">
            ${SITE.name} &middot; ${SITE.address}<br/>
            ${SITE.org} &middot; <a href="mailto:${SITE.email}" style="color:#357592;">${SITE.email}</a>
          </td>
        </tr>
      </table>
    </td></tr>
...
function detailRows(d: BookingEmailData, labels: Record<string, string>): string {
  const row = (k: string, v: string) =>
    `<tr>
      <td style="padding:6px 12px 6px 0;font-family:Inter,'Helvetica Neue',Arial,sans-serif;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#4A5F6E;white-space:nowrap;">${k}</td>
      <td style="padding:6px 0;font-size:15px;">${v}</td>
    </tr>`;
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:12px 0;border-left:3px solid #448BAB;padding-left:14px;">
...
export function hostelNotificationEmail(d: BookingEmailData): {
  ...
  const contact = [
    `<a href="mailto:${d.guestEmail}" style="color:#357592;">${d.guestEmail}</a>`,
  ...
  const body = `
    ...
    <p style="margin:16px 0 0;line-height:1.6;font-family:Inter,'Helvetica Neue',Arial,sans-serif;font-size:12px;color:#4A5F6E;">Lingua ospite: ${d.locale.toUpperCase()} — rispondere per confermare o rifiutare.</p>`;
  return {
